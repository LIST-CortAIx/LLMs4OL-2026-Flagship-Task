...
Usage
-----
    python scripts/prepare_data.py --scope whole       # final submission artifacts
    python scripts/prepare_data.py --scope split       # dev artifacts (after split_data.py)
    python scripts/prepare_data.py --scope both
    python scripts/prepare_data.py --scope whole --no-index        # gold only
"""
...
def do_split(args) -> None:
    ...
    if not (train_p.exists() and test_p.exists()):
        sys.exit(f"Split files not found in {sd} — run scripts/split_data.py first.")
